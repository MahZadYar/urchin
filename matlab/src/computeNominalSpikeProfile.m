function [rBaseNominal, rTipNominal, alphaConeNominal, zTipSeamNominal] = computeNominalSpikeProfile(coreRadius, spikeLength, tipSphereRadius, conicalityEffective, rBaseLimitLocal, scale)
%COMPUTENOMINALSPIKEPROFILE Baseline spike profile honoring tip tangency and conicality.
%
%   [rBaseNominal, rTipNominal, alphaConeNominal, zTipSeamNominal] =
%   COMPUTENOMINALSPIKEPROFILE(coreRadius, spikeLength, tipSphereRadius,
...
%   tangency limits imposed by the spherical tip and core geometry.
    ...
    if tipSphereRadius > 0 && rBaseLimitLocal > 0
        ...
        for iter = 1:8
            [rTipCandidate, ~, ~, tangentValid] = solveTipTangent(rBase, coreRadius, zTipCenter, tipSphereRadius);
            if ~tangentValid
        ...
        [rTipNominal, alphaConeNominal, zTipSeamNominal, tangentValid] = solveTipTangent(rBase, coreRadius, zTipCenter, tipSphereRadius);
        if ~tangentValid
