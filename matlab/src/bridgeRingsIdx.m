function Fpatch = bridgeRingsIdx(inner_idx, outer_idx, V, orientation)
%BRIDGERINGSIDX Zipper triangulation between two index loops.
%   Fpatch = BRIDGERINGSIDX(inner_idx, outer_idx, V, orientation)
...
%   winding order, degeneracies, and greedy diagonal selection.
    ...
    inner_vecs = V(inner_idx, :) - center_point;  % Implicit expansion
    ...
    distances_sq = sum((V(outer_idx, :) - v_inner_start).^2, 2);  % Implicit expansion
