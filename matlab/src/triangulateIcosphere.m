function [V, F] = triangulateIcosphere(R, subdivisions)
%TRIANGULATEICOSPHERE Generate a geodesic icosphere mesh.
%   [V, F] = TRIANGULATEICOSPHERE(R, subdivisions) creates a geodesic
...
%   the sphere of radius R, and faces contains triangle indices.
    ...
    for s = 1:subdivisions
        % Use dictionary for modern MATLAB (R2022b+); dictionary is faster than containers.Map
...
%MIDPOINTINDEX Return the vertex index of the edge midpoint, caching results.
    if a > b
        key = string(sprintf('%d_%d', b, a));
    else
        key = string(sprintf('%d_%d', a, b));
    end
